...
# Changes one of the dimenstions (H or W) of the video to a given dimention 
class Scale:
    ...
    def __call__(self, imgmap):
        ...
        toPIL=ToPIL()          # from tensor to PIL
        imgmap=toPIL(imgmap)
        ...
        if isinstance(self.size, int):
            ...
            if (w <= h and w == self.size) or (h <= w and h == self.size):
                return  toTensor(imgmap)
            if w < h:
                ow = self.size
                oh = int(self.size * h / w)
                return toTensor([i.resize((ow, oh), self.interpolation) for i in imgmap])
            else:
                oh = self.size
                ow = int(self.size * w / h)
                return toTensor([i.resize((ow, oh), self.interpolation) for i in imgmap])
        else:
            return toTensor([i.resize(self.size, self.interpolation) for i in imgmap])
...
class CenterCrop:
    ...
    def __call__(self, imgmap):
        ...
        x1 = int(round((w - tw) / 2.))
        y1 = int(round((h - th) / 2.))
        return toTensor([i.crop((x1, y1, x1 + tw, y1 + th)) for i in imgmap])
...
class FiveCrop:
    ...
    def __call__(self, imgmap):
        ...
        if self.where == 1:
            return toTensor([i.crop((0, 0, tw, th)) for i in imgmap])
        elif self.where == 2:
            return toTensor([i.crop((w-tw, 0, w, th)) for i in imgmap])
        elif self.where == 3:
            return toTensor([i.crop((0, h-th, tw, h)) for i in imgmap])
        elif self.where == 4:
            return toTensor([i.crop((w-tw, h-tw, w, h)) for i in imgmap])
        elif self.where == 5:
            x1 = int(round((w - tw) / 2.))
            y1 = int(round((h - th) / 2.))
            return toTensor([i.crop((x1, y1, x1 + tw, y1 + th)) for i in imgmap])
            ...
            for attempt in range(10):
                ...
                if self.consistent:
                    ...
                    if w <= img1.size[0] and h <= img1.size[1]:
                        ...
                        return toTensor([i.resize((self.size, self.size), self.interpolation) for i in imgmap])
                else:
                    result = []
                    ...
                    return toTensor([i.resize((self.size, self.size), self.interpolation) for i in result]) 

            # Fallback
            scale = Scale(self.size, interpolation=self.interpolation)
            crop = CenterCrop(self.size)
            return toTensor(crop(scale((imgmap))))                             # TypeError 
        else: #don't do RandomSizedCrop, do CenterCrop
            crop = CenterCrop(self.size)
            return toTensor(crop((imgmap)))                                    # TypeError
    ...
    def __call__(self, imgmap):
        ...
        if self.consistent:
            if random.random() < self.threshold:
                return toTensor([i.transpose(Image.FLIP_LEFT_RIGHT) for i in imgmap])
            else:
                return toTensor(imgmap)
        else:
            result = []
            for idx, i in enumerate(imgmap):
                ...
                if th < self.threshold:
                    ...
                    result.append(i) 
            assert len(result) == len(imgmap)
            return toTensor(result) 

# If you set the seq_len > 0 --> consistent=False Attention !!!
class RandomGray:
    ...
    def __call__(self, imgmap):
        ...
        if self.consistent:
            if random.random() < tmp_p:     
                return toTensor([self.grayscale(i) for i in imgmap])
            else:
                return toTensor(imgmap)
        else:
            result = []
    ...
    def __call__(self, imgmap):
        ...
        if random.random() < self.threshold: # do ColorJitter
            if self.consistent:
                ...
                return toTensor([transform(i) for i in imgmap])
            else:
                if self.seq_len == 0:
                    return toTensor([self.get_params(self.brightness, self.contrast, self.saturation, self.hue)(img) for img in imgmap])
                else:
                    result = []
...
# In this transforamtion consistent has to be True or An error will accure
    ...
    def __call__(self, imgmap):
        ...
        if random.random() < self.threshold: # do RandomRotation
            if self.consistent:
                ...
                return toTensor([i.rotate(deg, expand=True) for i in imgmap])
            else:
                return toTensor([i.rotate(np.random.randint(-self.degree, self.degree, 1)[0], expand=True) for i in imgmap])
...
class ToTensor:
    def __call__(self, imgmap):
        ...
        vid_tensor_list=[totensor(i) for i in imgmap] #<class 'list'> of lenght T of many tensors torch.Size([C, H, W])
        vid_tensor=torch.stack(vid_tensor_list).permute(1,0,2,3)       #torch.Size([C,T,H,W])
        return vid_tensor

class ToPIL:
    def __call__(self, imgmap):
        # takes a tensor of shape ([C,T,H,W]) 
        #convert it to shape ([T,C,H,W]) and outputs PIL images in a list of length T
        imgmap=imgmap.permute(1,0,2,3)
...
# These values are not from UCF101 dataset, they need to be updated
class Normalize:
    ...
    def __call__(self, imgmap):
        ...
        return toTensor([normalize(i) for i in imgmap])
