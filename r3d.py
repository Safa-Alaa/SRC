...
#https://github.com/DAVEISHAN/TCLR/blob/main/tclr_pretraining/r3d.py
import torch
...
class VideoResNet(nn.Module):
    ...
    def __init__(self, block, conv_makers, layers,
        ...
        ###self.feature_pool = nn.MaxPool3d(kernel_size=(4, 4, 4), stride=(2, 2, 2))
        self.feature_pool = nn.AdaptiveAvgPool3d((None, 2, 2))
        # init weights
    ...
    def forward(self, x):
        with autocast():
            ...
            return_conv = True
            if return_conv==True:
                #print (torch.reshape(x, (x.shape[0], -1)).shape)
                ###xx = self.avgpool(xx)
                xx = self.feature_pool(xx)
                ###print((torch.reshape(xx, (xx.shape[0], -1))).shape)
                return torch.reshape(xx, (xx.shape[0], -1))
